...
//권한이나 인증이 필요함 특정 주소를 요청했을 때 위 필터를 무조건 타게 돼있음
//만약에 권한 인증이 필요한 주소가 아니라면 이 필터를 안탐
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    private NewUserRepository newUserRepository;

    public JwtAuthorizationFilter(AuthenticationManager authenticationManager, NewUserRepository newUserRepository) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        //서명이 정상적으로 됨
        if(username != null) {
            NewUser newUserEntity = newUserRepository.findByUsername(username);

            PrincipalDetails principalDetails = new PrincipalDetails(newUserEntity);
